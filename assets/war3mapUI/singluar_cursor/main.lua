...
this.onStart(function()
    local stage = this.stage()
    ---@param ab Ability
    stage.cursorCatcher = function(x, y, radius, limit, ab)
        local g = group.catch({
            key = "Unit",
    ...
    japi.Refresh("singluar_cursor", function()
                        ...
                        if (isProhibiting or coolDownRemain > 0 or isObject(bindUnit, "Unit") == false) then
                            ...
                            if (rx < 0.015 or rx > 0.784 or ry < 0.02 or ry > 0.577) then
                                ...
                                if (tt == ABILITY_TARGET_TYPE.TAG_U or tt == ABILITY_TARGET_TYPE.TAG_L) then
                                    ...
                                    local catch = stage.cursorCatcher(tx, ty, 120, 5, ab)
                                    local closest
                                    if (#catch > 0) then
                                ...
                                elseif (tt == ABILITY_TARGET_TYPE.TAG_R) then
                                    ...
                                    if (ry >= 0.1343) then
                                        ...
                                        local radiusUnits = stage.cursorCatcher(tx, ty, tmpData.radius.range, 30, ab)
