...
_singluarSetTooltipsBuilder = {
    ...
    ---@type fun(idx:number,whichPlayer:Player,lvOffset:number):table|nil
    ability = function(idx, whichPlayer, lvOffset)
        local selection = whichPlayer.selection()
        if (selection == nil or selection.isDead()) then
            return nil
        end
        local ab = selection.abilitySlot().storage()[idx]
        ...
        local lv = (lvOffset or 0) + ab.level()
        local tips
        if (lvOffset > 0) then
    ...
    ---@type fun(idx:number,whichPlayer:Player):table|nil
    item = function(idx, whichPlayer)
        local selection = whichPlayer.selection()
        if (false == isObject(selection, "Unit") or selection.isDestroy()) then
            return nil
        end
        local it = selection.itemSlot().storage()[idx]
        if (isObject(it, "Item") == false) then
            return nil
        end
        local icons = {
            { 'lumber', 'C49D5A', '木' },
    ...
    ---@type fun(idx:number,whichPlayer:Player):table|nil
    warehouse = function(idx, whichPlayer)
        local it = whichPlayer.warehouseSlot().storage()[idx]
        if (isObject(it, "Item") == false) then
            return
        end
        local icons = {
        ...
        local res = it.worth()
        local cale = Game().worthCale(res, whichPlayer.sell() * 0.01)
        for _, c in ipairs(icons) do
            local key = c[1]
            local color = c[2]
            local uit = c[3]
